...
test('Форма "Написать отзыв" — отправка с видео (1:59): видео загружается, отзыв публикуется и отображается в видео-галерее', async ({ page, context }) => {
  ...
  try {
    ...
    // 6. Проверяем в панели администратора (в отдельной вкладке)
    ...
    // Навигация на другую страницу и обратно нужна чтобы Vue Router заново смонтировал
    // компонент и получил свежие данные из API (простой reload даёт кешированный контент CDN)
