...
const VIDEO_PATH_VERTICAL = path.resolve(__dirname, 'shuvaeva-vertical.mov');
...
const REVIEW_TEXT   = 'Проверка отправки отзыва с вертикальным видео — автотестирование';
const REVIEW_SNIPPET = 'с вертикальным видео — автотестирование';
...
test('Форма "Написать отзыв" — отправка вертикального видео: видео загружается, отзыв публикуется и отображается в видео-галерее', async ({ page, context }) => {
  ...
  try {
    ...
    // 3. Загружаем вертикальное видео через файловый диалог
    ...
    await fileChooser.setFiles(VIDEO_PATH_VERTICAL);
    console.log('[test] Вертикальное видео передано в диалог выбора файлов');
    ...
    console.log('[test] Результат загрузки вертикального видео:', uploadResult);
    if (uploadResult === 'error') {
      throw new Error('Форма отклонила вертикальное видео с сообщением об ошибке');
    }
    console.log('[test] ✓ Вертикальное видео загружено');
    ...
    // 6. Ждём пока кнопка станет активной — MOV-файлы дольше загружаются на сервер:
    // превью появляется раньше, чем завершается backend-upload.
    ...
    console.log('[test] ✓ Отзыв с вертикальным видео виден на странице /otzyvy');
    ...
    console.log('[test] ✓ Видео-отзыв отображается в галерее на /otzyvy');
