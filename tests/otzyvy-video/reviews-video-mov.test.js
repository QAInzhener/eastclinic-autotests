...
test('Форма "Написать отзыв" — отправка с видео MOV (1:59): видео загружается, отзыв публикуется и отображается в видео-галерее', async ({ page }) => {
  ...
  try {
    ...
    // 9. Проверяем на /otzyvy (акции → назад, до 3 попыток)
    ...
    // 10. Видео-карточка отображается в галерее /otzyvy
    ...
    // 11. Клик на карточку открывает плеер
