...
test('Форма "Записаться" (шапка) — ссылки на согласия/политики открываются корректно', async ({ page }) => {
  ...
  for (const linkText of LINKS) {
    ...
    try {
      ...
      // либо к скачиванию файла (собственный PDF-эндпоинт). Слушатели событий должны
      // быть зарегистрированы ДО клика — поэтому клик и ожидание запускаются вместе
      // через Promise.all, а не click(), а затем await ожидания (тогда ждать уже нечего).
      ...
      if (outcome.type === 'download') {
        ...
        // Ссылка на собственный PDF-эндпоинт не всегда триггерит событие 'download' —
        // иногда Chrome открывает PDF прямо во вкладке через встроенный просмотрщик.
        // В этом случае document.body.innerText пуст (PDF рендерится плагином, а не
        // текстом DOM), поэтому одной проверки innerText недостаточно — дополнительно
        // смотрим document.contentType: для НЕ-html документов (application/pdf и т.п.)
        // непустой innerText не требуется, достаточно того, что документ реально загрузился.
        const info = await popup.evaluate(() => ({
          contentType: document.contentType || '',
          hasContent: (document.body?.innerText?.length ?? 0) > 0,
        })).catch(() => ({ contentType: '', hasContent: false }));

        const isBrowserErrorPage = /^chrome-error:\/\//.test(finalUrl);
        const isNonHtmlDocument = info.contentType && info.contentType !== 'text/html';
        const loaded = !isBrowserErrorPage && finalUrl && finalUrl !== 'about:blank' && (info.hasContent || isNonHtmlDocument);

        if (!loaded) {
          const reason = isBrowserErrorPage
            ? 'браузер показал страницу ошибки сети вместо целевого сайта (ссылка не открывается)'
            : `новая вкладка не открыла страницу (осталась на "${finalUrl || 'about:blank'}")`;
          failed.push({ name: linkText, href, reason });
          console.log(`  ✗ ${linkText} — ${href} — вкладка не открыла контент (url="${finalUrl}")`);
        } else {
          console.log(`  ✓ ${linkText} — ${finalUrl} (внешняя ссылка, вкладка открылась${isNonHtmlDocument ? `, ${info.contentType}` : ''})`);
        }
        await popup.close();
      }
    } catch (e) {
