...
test('Форма "Написать отзыв" — заполняется, отправляется, публикуется и отображается на странице отзывов', async ({ page, context }) => {
  test.setTimeout(360000);
  let reviewSubmitted = false;
  let reviewPublished = false;

  // Отдельная вкладка для админ-панели: делит куки/сессию логина с основной страницей
  // (context общий), но имеет свою историю навигации. Иначе «клик Акции → goBack()»
  // в publishAndVerify мог случайно вернуть не на /otzyvy, а в засорённую переходами
  // внутри SPA-админки историю того же таба.
  ...
  try {
    ...
    // 2. Ждём появления отзыва в панели администратора (в отдельной вкладке)
    ...
    // 3. Публикуем в админке и проверяем на публичной странице
    reviewPublished = true;
    await publishAndVerify(adminPage, page);
...
// adminPage — вкладка админ-панели (публикация), page — вкладка публичного сайта (проверка).
// Разделены намеренно: обе используют навигацию (клики, goBack), и если бы это была одна
// и та же вкладка, история навигации из SPA-админки могла бы сбить goBack() на публичной
// странице с толку — он вернул бы не на /otzyvy, а куда-то внутрь админки.
async function publishAndVerify(adminPage, page) {
  // Переходим в раздел Отзывы (мы уже залогинены после checkReviewInAdmin)
  await adminPage.getByRole('link', { name: 'Отзывы' }).click();
  ...
  const info = await findReviewRow(adminPage);
  ...
  const row = adminPage.locator('tr').nth(info.idx);
