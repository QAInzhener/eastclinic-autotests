...
test('Форма "Записаться" — заполняется и отправляется', async ({ page }) => {
  ...
  await Promise.race([
    ...
    page.getByText(/ошибка сервера|попробуйте позже/i)
      .waitFor({ state: 'visible', timeout: 10000 })
      .then(() => { throw new Error('Сервер вернул ошибку при отправке формы "Записаться"'); }),
  ]);
...
test('Форма "Обратная связь" — заполняется и отправляется', async ({ page }) => {
  ...
  await Promise.race([
    ...
    page.getByText(/ошибка сервера|попробуйте позже/i)
      .waitFor({ state: 'visible', timeout: 10000 })
      .then(() => { throw new Error('Сервер вернул ошибку при отправке формы "Обратная связь"'); }),
  ]);
...
test('Форма "Написать директору" — заполняется и отправляется', async ({ page }) => {
  ...
  await Promise.race([
    ...
    page.getByText(/ошибка сервера|попробуйте позже/i)
      .waitFor({ state: 'visible', timeout: 10000 })
      .then(() => { throw new Error('Сервер вернул ошибку при отправке формы "Написать директору"'); }),
  ]);
