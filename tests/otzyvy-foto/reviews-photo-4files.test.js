...
test('Форма "Написать отзыв" — загрузка и отправка четырёх фото (JPG, JPEG, PNG, WebP): все фото загружаются, отзыв публикуется', async ({ page }) => {
  ...
  try {
    ...
    // Ищем карточку нашего отзыва по тексту и берём первое фото в ней
    const reviewCard = page.locator('[class*="review"], article, li, div')
      .filter({ hasText: REVIEW_TEXT }).first();
    const cardVisible = await reviewCard.isVisible({ timeout: 5000 }).catch(() => false);

    let galleryThumbnail;
    if (cardVisible) {
      galleryThumbnail = reviewCard
        .locator('img:not([class*="avatar"]):not([class*="logo"]), [class*="media-item"], [class*="photo"], [class*="thumb"]')
        .first();
    } else {
      galleryThumbnail = page.locator('.media-item').first();
    }

    await galleryThumbnail.waitFor({ state: 'visible', timeout: 10000 });
    await galleryThumbnail.scrollIntoViewIfNeeded();
    console.log('[test] ✓ Фото-миниатюра найдена в карточке отзыва');

    // Клик открывает просмотрщик
    await galleryThumbnail.click();
    ...
    const viewerOpened = await page.waitForFunction(
      () => {
        const candidates = [...document.querySelectorAll(
          ...
          '[class*="lightbox"], [class*="viewer"], [class*="carousel"]'
        )];
        return candidates.some(m => {
          const s = getComputedStyle(m);
          return s.display !== 'none' && s.visibility !== 'hidden' && m.querySelector('img');
        });
      },
      { timeout: 10000 }
    ).then(() => true).catch(() => false);

    expect(viewerOpened, 'Просмотрщик фото должен открыться при клике на галерею').toBe(true);
    console.log('[test] ✓ Просмотрщик фото открылся');

    // Пролистываем все 4 фото (→ × 3)
    for (let photoIdx = 1; photoIdx < 4; photoIdx++) {
      const nextBtn = page.locator(
        '[class*="next"]:not([disabled]), button[aria-label*="Next"], button[aria-label*="next"], button[aria-label*="след"]'
      ).first();
