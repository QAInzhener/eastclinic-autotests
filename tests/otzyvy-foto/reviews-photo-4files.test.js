...
test('Форма "Написать отзыв" — загрузка и отправка четырёх фото (JPG, JPEG, PNG, WebP): все фото загружаются, отзыв публикуется', async ({ page }) => {
  ...
  try {
    ...
    // Медленная прокрутка для загрузки lazy-контента
    ...
    // Найти первое фото в карточке нашего отзыва через JavaScript:
    // ищем элементы с классом "review*", содержащие текст отзыва и хотя бы одно фото
    const thumbHandle = await page.evaluateHandle((text) => {
      const candidates = [...document.querySelectorAll('[class*="review"], article')];
      for (const el of candidates) {
        if (!el.textContent.includes(text)) continue;
        // Ищем изображения внутри этого элемента
        const imgs = el.querySelectorAll('img[src]:not([src=""])');
        if (imgs.length >= 1) return imgs[0];
        // На некоторых сайтах фото-слоты не являются <img> — пробуем [class*="photo"], [class*="media"]
        const thumb = el.querySelector('[class*="photo"], [class*="media"], [class*="thumb"]');
        if (thumb) return thumb;
      }
      return null;
    }, REVIEW_TEXT);

    const thumbEl = thumbHandle ? thumbHandle.asElement() : null;
    if (!thumbEl) {
      throw new Error(
        `Не удалось найти фото в карточке отзыва на странице /otzyvy.\n` +
        `Проверьте, что отзыв "${REVIEW_TEXT.slice(0, 40)}..." опубликован и виден на странице.`
      );
    }

    // Прокрутка к фото и пауза, чтобы это было видно
    await thumbEl.scrollIntoView({ behavior: 'smooth', block: 'center' });
    ...
    // Пролистываем все 4 фото: нажимаем → три раза, пауза 1.5 с между фото
    for (let photoIdx = 1; photoIdx < 4; photoIdx++) {
      const nextBtn = page.locator(
        '[class*="next"]:not([disabled]), [class*="arrow"]:not([disabled]), ' +
        'button[aria-label*="Next"], button[aria-label*="next"], button[aria-label*="след"]'
      ).first();
      const clickedNext = await nextBtn.click({ timeout: 2000 }).then(() => true).catch(() => false);
      if (!clickedNext) {
