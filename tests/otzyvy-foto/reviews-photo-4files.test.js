...
test('Форма "Написать отзыв" — загрузка и отправка четырёх фото (JPG, JPEG, PNG, WebP): все фото загружаются, отзыв публикуется', async ({ page }) => {
  ...
  try {
    ...
    // Находим открытый плеер, чтобы искать кнопку «следующее» только внутри него
    const viewer = page.locator(
      '[role="dialog"], [class*="modal"], [class*="overlay"], ' +
      '[class*="lightbox"], [class*="viewer"], [class*="carousel"], [class*="gallery"]'
    ).filter({ has: page.locator('img') }).first();

    // Пролистываем все 4 фото: нажимаем шеврон → три раза, пауза 1.5 с между фото
    for (let photoIdx = 1; photoIdx < 4; photoIdx++) {
      // Ищем кнопку «следующее» строго внутри плеера
      const nextBtn = viewer.locator(
        '[class*="next"], [class*="right"], [class*="chevron"], ' +
        '[class*="arrow"], button[aria-label*="Next"], button[aria-label*="next"]'
      ).first();
      const clicked = await nextBtn.click({ timeout: 3000 }).then(() => true).catch(() => false);
      if (!clicked) {
        // Кнопка не найдена — используем клавишу стрелки
        await page.keyboard.press('ArrowRight');
        console.log(`[test] Кнопка «следующее» не найдена — использована стрелка →`);
      }
      await page.waitForTimeout(1500);
