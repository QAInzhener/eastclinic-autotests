...
test('Форма "Написать отзыв" — загрузка и отправка четырёх фото (JPG, JPEG, PNG, WebP): все фото загружаются, отзыв публикуется', async ({ page, context }) => {
  ...
  try {
    ...
    // Проверяем в панели администратора (в отдельной вкладке)
    ...
    // Ждём полного завершения анимации открытия плеера перед первым листанием
    await page.waitForTimeout(1000);

    // Пролистываем все 4 фото.
    // Шеврон «вправо» — это div/span (не button) снаружи белого окна плеера.
    // Вьюпорт всегда 1280×720, плеер центрирован → координаты шеврона стабильны.
    const vp = page.viewportSize() || { width: 1280, height: 720 };

    for (let photoIdx = 1; photoIdx < 4; photoIdx++) {
      let clicked = false;

      // Вариант 1: ищем div/span/a с типичными классами шеврона правее 80% экрана
      const candidates = page.locator('[class*="next"], [class*="chevron"], [class*="arrow"]');
      ...
      for (let j = 0; j < total && !clicked; j++) {
        ...
        if (!box)                              continue;
        if (box.x      <  vp.width  * 0.80)   continue; // правее 80% (>1024px)
        if (box.x + box.width  > vp.width)     continue; // в пределах вьюпорта
        if (box.y      <  vp.height * 0.20)    continue; // не шапка
        if (box.y + box.height > vp.height)    continue;
        clicked = await el.click({ timeout: 2000 }).then(() => true).catch(() => false);
        if (clicked) console.log(`[test] Шеврон (div/span) → (x=${Math.round(box.x)}, y=${Math.round(box.y)}, w=${Math.round(box.width)})`);
      }

      // Вариант 2: клик по координатам — шеврон всегда у правого края плеера
      // (из скриншота: ~88.5% ширины × ~53% высоты вьюпорта 1280×720 → x≈1133, y≈382)
      if (!clicked) {
        const cx = Math.round(vp.width * 0.885);
        const cy = Math.round(vp.height * 0.530);
        await page.mouse.click(cx, cy);
        clicked = true;
        console.log(`[test] Шеврон (координаты) → (x=${cx}, y=${cy})`);
      }

      // Пауза 2.5 с: 0.5 с анимация перехода + 2 с на просмотр фото
      await page.waitForTimeout(2500);
