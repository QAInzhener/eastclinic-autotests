...
test('Форма "Написать отзыв" — загрузка и отправка четырёх фото (JPG, JPEG, PNG, WebP): все фото загружаются, отзыв публикуется', async ({ page }) => {
  ...
  try {
    ...
    // Шеврон «вправо» — белый круглый элемент СНАРУЖИ белого окна плеера.
    // Критерии: правее 85% вьюпорта, внутри вьюпорта, не в шапке, ширина 25–80px.
    ...
    for (let photoIdx = 1; photoIdx < 4; photoIdx++) {
      ...
      for (let j = 0; j < total && !clicked; j++) {
        ...
        if (!box)                               continue;
        if (box.x      <= vp.width  * 0.85)    continue; // правее 85%  (>1088px при 1280)
        if (box.x + box.width  > vp.width)      continue; // не за правым краем
        if (box.y      <  vp.height * 0.25)     continue; // не в шапке  (>180px при 720)
        if (box.y + box.height > vp.height)     continue; // не за нижним краем
        if (box.width  < 25 || box.width > 80)  continue; // небольшая кнопка
        clicked = await btn.click({ timeout: 2000 }).then(() => true).catch(() => false);
        if (clicked) console.log(`[test] Шеврон → нажат (x=${Math.round(box.x)}, y=${Math.round(box.y)}, w=${Math.round(box.width)})`);
      }
