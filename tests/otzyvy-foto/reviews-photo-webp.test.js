...
const PHOTO_PATH = path.resolve(__dirname, 'muzhchina-ocean-1280x720.webp');
...
const REVIEW_TEXT    = 'Проверка отправки отзыва с фото WebP — автотестирование';
const REVIEW_SNIPPET = 'с фото WebP — автотестирование';
...
test('Форма "Написать отзыв" — отправка фото WebP (1280×720): фото загружается, отзыв публикуется и отображается на странице', async ({ page, context }) => {
  ...
  try {
    ...
    console.log('[test] Фото WebP передано в диалог выбора файлов');
    ...
    console.log('[test] Результат загрузки фото WebP:', uploadResult);
    if (uploadResult === 'error') throw new Error('Форма отклонила фото WebP с сообщением об ошибке');
    console.log('[test] ✓ Фото WebP загружено');
    ...
    console.log('[test] ✓ Отзыв с фото WebP отправлен');
    ...
    if (!found) throw new Error(`Отзыв с фото WebP не найден на странице ${REVIEWS_PAGE}`);
    console.log('[test] ✓ Отзыв с фото WebP виден на странице /otzyvy');
