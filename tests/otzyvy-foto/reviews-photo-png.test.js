...
test('Форма "Написать отзыв" — отправка фото PNG вертикального (960×1440): фото загружается, отзыв публикуется и отображается на странице', async ({ page }) => {
  ...
  try {
    ...
    console.log('[test] Фото PNG (вертикальное) передано в диалог выбора файлов');
    ...
    console.log('[test] Результат загрузки фото PNG:', uploadResult);
    if (uploadResult === 'error') throw new Error('Форма отклонила фото PNG с сообщением об ошибке');
    console.log('[test] ✓ Фото PNG (вертикальное) загружено');
    ...
    console.log('[test] ✓ Отзыв с фото PNG (вертикальное) виден на странице /otzyvy');
